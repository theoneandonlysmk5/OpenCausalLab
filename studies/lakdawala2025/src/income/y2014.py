"""2014 Income harmonization — EH_Income_2014.do."""
...
def harmonize_income_2014(raw_path: Path | None = None) -> pd.DataFrame:
    df = read_persona(2014, raw_path)
    ...
    out["aguinaldo_yearly_main"] = to_numeric(df["s6c_26a"])  # Stata: s6c_26a not 26b
    out["comision_monthly_main"] = monthly_wage(df["s6c_27a_1"], df["s6c_27a_2"])
    out["overtime_monthly_main"] = monthly_wage(df["s6c_27b_1"], df["s6c_27b_2"])
    # 2014 excludes aguinaldo_monthly from extra_wages_main
    out["extra_wages_main"] = rowtotal(
        out, ["bonus_monthly_main", "comision_monthly_main", "overtime_monthly_main"]
    )
    out["extra_wages_sec"] = zero_if_missing(to_numeric(df["s6f_42a_1"]) / 12)
    ...
    for kind, letter in zip(kinds, letters):
        ...
        out[col] = zero_if_missing(
            monthly_rev(df[f"s6c_30{letter}_3"], df[f"s6c_30{letter}_2"])
        )
        inkind_cols.append(col)
    out["inkind_payments_main"] = rowtotal(out, inkind_cols)
    out["inkind_payments_sec"] = rowtotal(df, ["s6f_42b_1", "s6f_42c_1"]) / 12
    ...
    for occ, base in [("main", "d_32"), ("sec", "f_44")]:
        for name, letter in zip(cost_names, cost_letters):
            ...
            out[col] = zero_if_missing(
                monthly_rev(df[f"s6{base}{letter}_1"], df[f"s6{base}{letter}_2"])
            )
    ...
    out["retirement"] = to_numeric(df["s7a_01a"])
    out["transfer_veterans"] = to_numeric(df["s7a_01b"])
    out["transfer_disability"] = to_numeric(df["s7a_01c"])
    out["transfer_widows"] = to_numeric(df["s7a_01d"])
    out["y_social_security"] = rowtotal(
        out, ["retirement", "transfer_veterans", "transfer_disability", "transfer_widows"]
    )
    out["y_elderly_transfer"] = zero_if_missing(df["s7a_01e_b"])
    ...
    for name, letter in [("family_asistance", "a"), ("people_incountry", "b")]:
        out[f"{name}_monthly"] = zero_if_missing(
            monthly_rev(df[f"s7b_05{letter}_1"], df[f"s7b_05{letter}_2"])
        )
    ...
    for name, letter in [
        ...
        out[name] = zero_if_missing(df[f"s7a_02{letter}"])
    out["y_int_assets_regular"] = rowtotal(out, ["revenues_interest", "revenues_renting", "revenues_other"])

    nr_map = {
        "revenues_rental_agric": "s7a_03a",
        "revenues_dividends": "s7a_03b",
        "revenues_rental_equip": "s7a_03c",
        "revenues_indemnization": "s7a_04a",
        "revenues_insurance": "s7a_04b",
        "revenues_other_nr": "s7a_04c",
    }
