"""2019 Persona harmonization — EH_Persona_2019.do."""
...
from ..stata_utils import inlist, inrange, replace_where, to_numeric
from .common import (
    binary_01,
...
def harmonize_persona_2019(raw_path: Path | None = None) -> pd.DataFrame:
    """Faithful Python translation of EH_Persona_2019.do."""
    df = read_persona(2019, raw_path)
    ...
    ocu_cat = make_ocu_cat(df, "s06b_14")
    out["ocu_cat"] = ocu_cat
    out["ocu_cat2"] = make_ocu_cat2(ocu_cat)

    out["hours_worked_day"] = make_hours_worked_day_2019(df)
    out["days_worked_week"] = to_numeric(df["s06b_22"])
    out["job_location"] = make_job_location(df, out["age"], category4_values=[9])

    # 2019 uses s05a_04 for enrollment (not s05a_05)
    out["enrollment"] = binary_01(df["s05a_04"])
    ...
    idioma = replace_where(idioma, 7.0, inlist(s02a_08, [995, 998]))
    ...
    out["number_workers"] = to_numeric(df["s06b_21a"])
    out["type_est"] = make_type_est(df)
    out["places_taxes2"] = make_places_taxes2(df, source_col="s06b_19a")
    ...
    return select_keep(out, PERSONA_2019_KEEP)
